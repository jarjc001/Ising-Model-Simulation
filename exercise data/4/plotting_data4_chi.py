...
T_Tc = df["T_Tc"]
rel_chi = df["relative_chi"]
mf_t_tc = df["mf_T/T_C"]
mf_rel_chi = df["mf_chi"]
...
plt.plot(T_Tc,rel_chi,'k.',markersize=7,label='Ising',linewidth=2.5)
plt.plot(mf_t_tc,mf_rel_chi,'r',markersize=7,label='Mean-Field',linewidth=2.5)


plt.show
plt.xlabel('T/T$_c$',fontsize=12)           #labels x axis as 'r (μm)'
plt.legend()
plt.ylabel('χ/χ$_{max}$',fontsize=12)    #labels y axis as 'Intensity (Wm^-2)'
